...
/* shadow 设定值.
 * Stage 8 (2026-05-06): 默认值改为安全占位 (Codex v2 问题 2):
 *   - 防止屏幕未收到主板第一帧前用户单击, 把硬编码默认值 (例如 36°C) commit 给主板;
 *   - hmi_commit_current_page 入口检查 shadow_valid, 占位时拒绝;
 *   - 仅 Stage 8 主板 (帧 len >= 38) 通过 byte 26-37 真正 seed 后, shadow_valid=1. */
static int16_t   s_set_temp_x10   = -999;
static uint16_t  s_set_hum_x10    = 0;
static uint16_t  s_set_o2_x10     = 0;
static uint16_t  s_set_co2_ppm    = 0;
...
/* Stage 8 (2026-05-06): hmi_seed_from_display 重写
 * - 加 len 参数 (Codex v2 问题 1): 仅当 len >= 38 才 seed setpoint
 * - seed 改用 byte 26-37 setpoint 字段 (Codex v1 问题 5), 不再用实时值
 * - len < 38 (旧主板) → shadow_valid=0, commit 被拒绝, 用户感知为"按了没反应"
 * - 已 valid 时不重 seed (避免覆盖用户编辑) — 与并发 last-writer-wins 语义一致 */
static void hmi_seed_from_display(const uint8_t *data, uint8_t len)
{
    if (s_hmi_shadow_valid) return;
    if (len < 38) {
        /* 旧主板 (Stage 7) 帧, 没有 setpoint 字段 → 拒绝 seed */
        s_hmi_shadow_valid = 0;
        return;
    }

    /* byte 26-37: setpoint 字段 */
    ...
    /* clamp 到合法范围 (防御主板帧错乱; 主板自己也已 clamp) */
...
/* Stage 8 新增: 显式失效 shadow.
 * 用于: 5s 超时 / 长按取消 / 单击 commit 后 → 让下次进 SET 页重新 seed 拉最新 setpoint. */
static void hmi_invalidate_shadow(void)
{
    s_hmi_shadow_valid = 0;
}

/* Stage 4: 8 页循环 (Stage 1 之前是 4 页)
 * Stage 8 (2026-05-06): 跳过 HMI_PAGE_SET_CO2 (Codex v2 问题 6).
 * 原因: target_co2 仍是 placeholder (主板 AlarmTask CO2 报警仍用固定 5000ppm),
 * 让用户编辑 CO2 阈值是误导. 等 Stage 9 实现 CO2 报警/控制后移除 do-while 跳过. */
...
/* Stage 4: 编码器旋转 → 当前页对应字段 ± 步长.
 * Stage 8 (2026-05-06) 重写 (Codex v1 问题 5):
 *   - 旋转期间只改本地 shadow, 不再立即发 send_param_set (避免实时下发 → enable 误触发)
 *   - 主板 setpoint 仅在用户单击 commit 时才被更新 (hmi_commit_current_page)
 *   - shadow_valid=0 时不响应旋转 (屏幕未收到主板 38B 帧, 没有合法 seed) */
static void hmi_apply_encoder_delta(int8_t delta)
{
    ...
    while (delta > 0) {
        ...
        case HMI_PAGE_SET_TEMP:
            if (s_set_temp_x10 < 400) s_set_temp_x10++;
            break;
        case HMI_PAGE_SET_HUMID:
            if (s_set_hum_x10 < 900) s_set_hum_x10 += 10;
            break;
        case HMI_PAGE_SET_O2:
            if (s_set_o2_x10 < 1000) s_set_o2_x10 += 10;
            break;
        case HMI_PAGE_SET_CO2:
            if (s_set_co2_ppm <= 5000 - 100) s_set_co2_ppm += 100;
            break;
        case HMI_PAGE_SET_FOG:
            if (s_set_fog_sec <= 3600 - 60) s_set_fog_sec += 60;
            break;
        case HMI_PAGE_SET_DISINFECT:
            if (s_set_disinf_sec <= 3600 - 60) s_set_disinf_sec += 60;
            break;
        default:
            break;
        }
    ...
    while (delta < 0) {
        switch (s_hmi_page) {
        case HMI_PAGE_SET_TEMP:
            if (s_set_temp_x10 > 100) s_set_temp_x10--;
            break;
        case HMI_PAGE_SET_HUMID:
            if (s_set_hum_x10 > 300) s_set_hum_x10 -= 10;
            break;
        case HMI_PAGE_SET_O2:
            if (s_set_o2_x10 > 210) s_set_o2_x10 -= 10;
            break;
        case HMI_PAGE_SET_CO2:
            if (s_set_co2_ppm >= 100) s_set_co2_ppm -= 100;
            break;
        case HMI_PAGE_SET_FOG:
            if (s_set_fog_sec >= 60) s_set_fog_sec -= 60;
            break;
        case HMI_PAGE_SET_DISINFECT:
            if (s_set_disinf_sec >= 60) s_set_disinf_sec -= 60;
            break;
        default:
            break;
        }
...
/* Stage 8 (2026-05-06): 提交当前 SET 页的 shadow 到主板.
 * 入口检查 shadow_valid (Codex v2 问题 2) — 屏幕未收到 38B 帧前拒绝 commit,
 * 防止默认占位值 (-999/0) 被错误写入 setpoint.
 * 返回 1 = 成功 commit, 0 = 拒绝 (shadow 无效或当前页非 SET). */
static uint8_t hmi_commit_current_page(void)
{
    if (!s_hmi_shadow_valid) return 0;
    switch (s_hmi_page) {
    case HMI_PAGE_SET_TEMP:
    ...
    case HMI_PAGE_SET_O2:
        ...
        return 1;
    case HMI_PAGE_SET_CO2:
        /* Stage 8: 此页已被 hmi_cycle_page 跳过, 通常用户走不到. 保留 commit 路径
         * 兼容 Stage 9 启用. target_co2 placeholder, commit 仅传值, 不影响报警. */
        send_param_set(0x06, s_set_co2_ppm);
        return 1;
    case HMI_PAGE_SET_FOG:
...
static void process_rx_frame(uint8_t cmd, const uint8_t *data, uint8_t len)
{
    ...
    case 0x06:  /* Encoder event from main board
                 ...
                 *
                 * Stage 8 (2026-05-06) 状态机重写 (Codex v1 问题 5/6 + v2 问题 2):
                 *   单击:
                 *     - LIVE 页        → 翻 SET_TEMP (开始编辑)
                 *     - RESET_O2 页    → 触发清累计 + 回 LIVE
                 *     - SET_xxx 页     → commit shadow + 翻下一页 (X2 链式编辑)
                 *                        commit 失败 (shadow_valid=0) 时静默忽略, 不翻页
                 *   长按:
                 *     - LIVE 页        → 清供氧累计 (b 方案保留)
                 *     - SET_xxx 页 / RESET_O2 → 取消编辑 (丢 shadow + 回 LIVE + 主板短鸣)
                 *   旋转:
                 *     - 仅 SET 页响应, 改本地 shadow, 不下发主板 */
        if (len >= 1) {
            uint8_t evt = data[0];
            if (evt == 0x01) {
                /* 单击 */
                if (s_hmi_page == HMI_PAGE_LIVE) {
                    hmi_cycle_page();
                } else if (s_hmi_page == HMI_PAGE_RESET_O2_TIME) {
                    send_timer_ctrl(0x03, 0x03, 0);   /* 供氧累计清零 */
                    s_hmi_page = HMI_PAGE_LIVE;
                    hmi_invalidate_shadow();
                    hmi_touch();
                } else {
                    /* SET_xxx 页 → commit + 翻下一页 (X2 链式) */
                    if (hmi_commit_current_page()) {
                        hmi_invalidate_shadow();   /* commit 后强制下次重 seed, 拉最新 setpoint */
                        hmi_cycle_page();
                    }
                    /* commit 失败 (shadow_valid=0) → 静默忽略, 不翻页, 让用户感知"按了没反应"
                     * (典型场景: 屏幕未收到主板 38B 帧, 或主板还在 Stage 7 旧版本) */
                }
            } else if (evt == 0x02) {
                /* 长按 */
                if (s_hmi_page == HMI_PAGE_LIVE) {
                    ...
                    /* SET / RESET 页 → 取消编辑 (Stage 8 新增) */
                    hmi_invalidate_shadow();
                    s_hmi_page = HMI_PAGE_LIVE;
                    hmi_touch();
                    /* 主板触发短鸣 200ms 提示用户取消成功 */
                    send_key_action(0x0A, 0x05);
                }
...
int main(void)
{
    ...
    while (1) {
        ...
        if (s_hmi_page != HMI_PAGE_LIVE && (tick_ms() - s_hmi_last_input_tick >= HMI_EDIT_TIMEOUT_MS)) {
            s_hmi_page = HMI_PAGE_LIVE;
            /* Stage 8 (Codex v1 问题 6): 超时清 shadow_valid, 让下次进 SET 页重新 seed
             * 拉最新 setpoint (避免用陈旧 shadow 覆盖 iPad 同期改动) */
            hmi_invalidate_shadow();
        }
