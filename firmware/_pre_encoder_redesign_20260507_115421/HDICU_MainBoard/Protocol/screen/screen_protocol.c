...
#include "stm32f1xx_hal.h" /* HAL_GetTick for cancel_beep_until_tick */
#include <string.h>
    ...
    case SCR_CMD_KEY_ACTION: {
        ...
        /* --- Long press actions (>2s) --- */
            ...
            case 0x0A: /* 编码器长按 = LIVE 页 → 供氧累计清零 (b 方案保留, frozen spec 4.4)
                        * Stage 8: SET 页长按由屏幕板转换为 action=0x05 取消事件, 不走这里 */
            {
        ...
        /* --- Stage 8 (2026-05-06): action=0x05 取消事件 (SET 页长按) --- */
        else if (action == 0x05) {
            if (key_id == 0x0A) {
                /* 编码器在 SET 页长按 → 屏幕板已撤回 shadow + 回 LIVE,
                 * 主板这里只触发 200ms 短鸣提示用户. 用 deadline tick 避免相位错位. */
                d->control.cancel_beep_until_tick = HAL_GetTick() + 200;
            }
        }
        app_data_unlock();
        break;
