...
typedef struct {
    ...
    /* === Stage 8 新增 (2026-05-06) ===
     * actuator 启用标志, 上电默认 0 (关).
     * 写 0x81/0x03 (任意温/湿/O2 setpoint) 后自动置 1.
     * 0x0B 出厂复位回到 0.
     * 不持久化 Flash, 重启回到 0.
     *
     * 控制逻辑:
     *   enable=0 时, 对应 controller 的闭环段被旁路 (output 强制安全态).
     *   报警 (TEMP_HIGH/LOW/HUMID/O2_CO2 中的 O2 部分) 同步跟随 enable.
     *   手动开放供氧 (open_o2) 和外部 O2 请求不受 enable_o2_ctrl 影响. */
    uint8_t  enable_temp_ctrl;
    uint8_t  enable_humid_ctrl;
    uint8_t  enable_o2_ctrl;
} Setpoints_t;
...
typedef struct {
    ...
    /* Stage 8 (2026-05-06): 编码器长按取消短鸣 deadline.
     * 屏幕板长按 SET 页 → 0x82 KeyAction(0x0A, 0x05) → 主板写 deadline = HAL_GetTick() + 200.
     * AlarmTask 100ms tick 比较 (int32_t)(deadline - now) > 0 判断是否响.
     * deadline 模式比"递减计数"避免 ControlTask 200ms tick 与 AlarmTask 100ms tick 的相位错位
     * (Codex v2 审查问题 10). 32-bit tick wrap 在 200ms 窗口下不会发生 (49 天周期). */
    uint32_t cancel_beep_until_tick;
} ControlState_t;
