...
typedef struct {
    ...
    /* Stage 8 v3 删除 (2026-05-07 重做):
     * cancel_beep_until_tick 字段 — 编码器长按取消短鸣 deadline.
     * 重做后, 编码器长按 SET 页是"撤销 setpoint 回滚到原值", 不再短鸣.
     * 因此该字段连带 KeyAction(0x0A, 0x05) 协议、AlarmTask 三优先级 buzzer 分支一起移除. */
} ControlState_t;
