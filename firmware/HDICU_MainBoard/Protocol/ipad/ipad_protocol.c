...
static void handle_write_params(const uint8_t *data, uint8_t len)
{
    ...
    if (err_code != IPAD_ERR_NONE) {
        ...
        /* 只有非取消字段才更新 setpoint
         * Stage 8 (2026-05-06): 写温/湿/O2 setpoint 时同步置 enable_xxx_ctrl=1
         * (与屏幕 0x81 路径对称, 实现"写参数即启用闭环"的产品语义) */
        if (cancel_temp)  { d->setpoint.target_temp     = temp;  d->setpoint.enable_temp_ctrl  = 1; }
        if (cancel_humid) { d->setpoint.target_humidity = humid; d->setpoint.enable_humid_ctrl = 1; }
        if (cancel_o2)    { d->setpoint.target_o2       = o2;    d->setpoint.enable_o2_ctrl    = 1; }
        if (cancel_co2)   d->setpoint.target_co2      = co2;
        /* target_co2 不绑 enable: AlarmTask CO2 报警仍用固定 5000ppm, target_co2 是 placeholder */
...
static void handle_factory_reset(void)
{
    ...
    /* Stage 8 (2026-05-06): 复位 enable 标志 — 出厂复位后必须重新写参数才会启动闭环
     * (Codex v1 问题 9). 与上电默认行为对齐. */
    d->setpoint.enable_temp_ctrl  = 0;
    d->setpoint.enable_humid_ctrl = 0;
    d->setpoint.enable_o2_ctrl    = 0;

    /* Stage 8 (Codex v2 问题 4): 同步清对应温/湿/O2 报警 latch + ack 状态.
     * 否则报警在 enable 关闭后仍残留, 屏幕仍闪烁视觉报警 (虽蜂鸣已被 buzz_mask 过滤).
     * CO2 / 心率 / SpO2 / COMM_FAULT 不受 enable 影响, 也不在此清. */
    d->alarm.alarm_flags &= ~(ALARM_TEMP_HIGH | ALARM_TEMP_LOW | ALARM_HUMID | ALARM_O2_CO2);
    d->alarm.acknowledged = false;
    /* delay counters 不持久化在 alarm 结构中 (在 AlarmTask 函数局部 static),
     * 由 enable=0 后下一 100ms tick 的 else 分支自然清零. */
