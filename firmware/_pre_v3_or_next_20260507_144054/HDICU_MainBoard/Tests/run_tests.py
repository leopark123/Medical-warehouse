...
def test_screen_frame():
    ...
    # Display data: cmd=0x01, 39 bytes (Stage 8: 26→39 — added setpoint + version+enable)
    data = bytes(39)
    frame = build_screen_frame(0x01, data)
    expected_cs = (0x01 + 39) & 0xFF  # = 0x28
    assert_eq(frame[0], 0xAA, "Screen head1")
    assert_eq(frame[1], 0x55, "Screen head2")
    assert_eq(frame[2], 0x01, "Screen cmd")
    assert_eq(frame[3], 39, "Screen len (Stage 8 = 39 bytes)")
    ...
    ipad_cs = (0xAA + 0x01 + 39 + sum(data)) & 0xFF
    screen_cs = (0x01 + 39 + sum(data)) & 0xFF
...
def test_alarm():
    """Alarm spec:
       ...
       - Stage 8 (2026-05-06): 蜂鸣过滤 — 仅 COMM_FAULT (B8) 触发蜂鸣,
         B1-B7 (温/湿/O2/CO2/HR/SpO2) latch + 视觉闪烁但 NOT 蜂鸣.
    ...
    TEMP_HIGH = 1 << 0
    COMM_FAULT = 1 << 6
    BUZZ_MASK = COMM_FAULT   # Stage 8: only COMM_FAULT triggers buzzer

    # Step 1: Temperature alarm triggers (Stage 8: latch but NOT buzzer)
    current_flags = TEMP_HIGH
    alarm_flags |= current_flags
    assert_eq(alarm_flags, TEMP_HIGH, "Alarm latched")
    buzzer = ((alarm_flags & BUZZ_MASK) != 0) and not acknowledged
    assert_true(not buzzer, "Stage 8: TEMP_HIGH alarm latched but buzzer SILENT (only COMM_FAULT buzzes)")

    # Step 2: User acknowledges (silent path same)
    acknowledged = True
    buzzer = ((alarm_flags & BUZZ_MASK) != 0) and not acknowledged
    assert_true(not buzzer, "Buzzer silenced after acknowledge (still silent)")
