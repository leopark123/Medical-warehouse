...
{
    /* === Stage 8 (2026-05-06): 湿控未启用 → 仅清 JIASHI ===
     * YASUO/FENGJI 由温控决定 (温控可能在 COOLING, 不能抢).
     * 与原 IDLE 路径一致 (line 91-94): 不动 YASUO/FENGJI.
     * 唯一例外: 旧 humid_state==DEHUMIDIFY 时湿控曾借用 YASUO/FENGJI, 此时必须释放,
     * 但仅在温控不需要它们 (即非 COOLING) 时才释放, 与现有 DEHUMIDIFY 退出语义对称. */
    if (!d->setpoint.enable_humid_ctrl) {
        ...
        *r &= ~(1U << BSP_RELAY_JIASHI_IO);
        if (d->control.humid_state == HUMID_STATE_DEHUMIDIFY &&
            ...
            *r &= ~((1U << BSP_RELAY_YASUO_IO) | (1U << BSP_RELAY_FENGJI_IO));
        }
