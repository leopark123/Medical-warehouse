...
int16_t ntc_adc_to_temp_x10(uint16_t adc_value)
{
    ...
    /* === Stage 8 redo v2 修订 (2026-05-07): 物理温度范围合理性检查 ===
     * 触发: dump 显示 temp_state 周期性切到 COOLING (sample 3, 5), 但 actual 只有 26.7°C.
     * 根因: NTC 物理偶尔短路/接触不良时, ADC 跳到 10~50 边界附近, 计算出 200~272°C 极高值.
     *       30 sample 滑动平均稀释后仍可能超过 upper(setpoint+1°C), 触发误切 COOLING → PE1 闪烁.
     * 修复: 限制 ntc_adc_to_temp_x10 输出范围到 [-40, 60]°C, 超出视为 invalid (-999).
     *       培养箱物理使用范围 10~40°C, 60°C 已是充足上限; -40°C 覆盖低温环境.
     *       滤波器跳过 -999, 单次抖动不再污染平均值. */
